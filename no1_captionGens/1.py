...
def main():
   ...
   #     args.optimizer, params=model.parameters(), lr=args.lr)  # args.lr = 0.1  float
    ...
    def forward(model, data, training=True, optimizer=None):
        ...
        perplexity = AverageMeter()  # <utils.AverageMeter object at 0x7fa6be6f1780
        ...
        for i, (imgs, (captions, lengths)) in enumerate(data):  #len(data)键值对个数 2587
            time.sleep(1)
            if i == 3:
                break
    ...
    for epoch in range(args.start_epoch, args.epochs): #(0,10)
        ...
        optimizer = adjust_optimizer(
            optimizer, epoch, regime)   # optimizer none
